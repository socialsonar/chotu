import { createRedis, createSql } from "./platform";
...
export default class ChotuImpl {
    config;
    onShutdown;
    sql;
    redis;
    ...
    constructor(config, onShutdown) {
        ...
        this.flusher = new PgFlusher(this.redis, this.repository, config.flushIntervalMs ?? 1000, this.logger);
        const registry = new StepRegistry(config.queues, config.stepQueues, config.workflows, {
        ...
        const lifecycle = new WorkflowLifecycle(this.stateStore, this.repository, this.fairQueue, registry, this.logger, hookRunner);
        const stepExecutor = new StepExecutor(lifecycle, registry, this.fairQueue, this.logger, hookRunner);
        const recovery = new RecoveryService(this.stateStore, this.repository, this.fairQueue, lifecycle, registry, this.logger, this.redis, this.instanceId);
        const workerPool = new QueueWorkerPool(this.fairQueue, this.stateStore, stepExecutor, recovery, registry, this.logger, this.instanceId, hookRunner);
        this.engine = new ChotuEngine(this.flusher, workerPool, lifecycle, recovery);
    }
    isStarted() {
        return this.started;
    }
...
//# sourceMappingURL=chotu.impl.js.map